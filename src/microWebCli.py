...
class MicroWebCli:
    ...
    def GETRequest(url, queryParams=None, auth=None, connTimeoutSec=10, socks5Addr=None):
        c = MicroWebCli(
            url, auth=auth, connTimeoutSec=connTimeoutSec, socks5Addr=socks5Addr)
        if queryParams:
    ...
    def POSTRequest(url, formData={}, auth=None, connTimeoutSec=10, socks5Addr=None):
        c = MicroWebCli(url, method='POST', auth=auth,
                        connTimeoutSec=connTimeoutSec, socks5Addr=socks5Addr)
    ...
    def OpenRequest(self,
        ...
        if self.Socks5Addr:
            ...
            if not isinstance(host, str) or not isinstance(port, int):
                raise Exception(
                    '"Socks5Addr" is incorrect ("%s", %s)' % self.Socks5Addr)
        else:
            host = self.Host
            port = self.Port
        self._response = None
        ...
        if self._auth:
            try:
                ...
                raise Exception(
                    'Error to apply authentication using %s' % type(self._auth))
        else:
        ...
        if len(formData) > 0:
            for param in formData:
                if param != '':
                    if data != '':
                        data += '&'
                    data += MicroWebCli._quote(param) + '=' + \
                        MicroWebCli._quote(formData[param])
    ...
    def GetResponse(self):
        if not self._response:
            self._response = MicroWebCli._response(
                self, self._socket, self._socketAddr)
    ...
    def QueryString(self):
        ...
        for param in self._queryParams:
            if param != '':
                if r != '':
                    r += '&'
                r += MicroWebCli._quote(param) + '=' + \
                    MicroWebCli._quote(self._queryParams[param])
        return r
    ...
    class _response:
        ...
        def ReadContent(self, size=None):
            try:
                ...
                self.Close()
                raise MemoryError(
                    'Error to read response content (%s)' % memEx)
            except:
                self.Close()
            return None
        ...
        def WriteContentToFile(self, filepath, progressCallback=None):
            ...
            buf = MicroWebCli._tryAllocByteArray(
                fSize if fSize and fSize < 1024 else 1024)
            if not buf:
            ...
            if sizeRem and sizeRem > 0:
                ...
                remove(filepath)
                raise Exception(
                    'Error to receive and save file (%s)' % filepath)
